...
=======
  ...
  useMotionValueEvent(scrollY, 'change', (sv) => {
    ...
    /* Phase E — handoff: labels gone, "Why NEXUS." slides off to the RIGHT
       and fades while the SAME circle eases from its active state into its
       CTA-backdrop state. Values land exactly on Phase F's so nothing pops. */
      ...
      driftY.set(Math.sin(p * Math.PI) * vh * 0.05); // dip down → recenter (no snap-back)
      centerOp.set(lerp(1, 0, clamp(p / 0.6)));      // text fades before it's fully gone
      whyX.set(lerp(0, size * 0.95, smooth(clamp(p / 0.85))));
      sweep.set(-ORBIT_SWEEP_DEG);
      orbitPhaseRaw.set(1);

    /* Phase F — CTA hold: circle is the centered backdrop (matches E's end). */
...
>>>>>>> 44b14a8 (Auto-push: nexus-ai/src/components/sections/orbit.tsx)
